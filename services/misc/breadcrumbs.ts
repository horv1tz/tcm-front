...
export class Breadcrumbs {
    ...
    constructor(init_data: Category[]) {
        this.map = init_data
        this.top_row = init_data.filter(e=>!e.parentId)
        this.list = init_data.filter(e=>e.parentId===this.top_row[0].id)
    ...
    deeper(id: number){
        ...
        this.list = this.map.filter(e=>e.parentId === id)
        return this
    }
    ...
    goBack(){
        ...
        this.top_row = this.map.filter(e=>e.parentId === this.trail[this.trail.length-2])
        return this
    }

    getChildrenOf(id: number){
        return this.map.filter(e=>e.parentId===id)
    }
}